...
from rsl_rl_MLE_stage2.networks import Memory
from rsl_rl_MLE_stage2.utils import resolve_nn_activation
...
class TerrainAwareStudentTeacher(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.student_encoder = self._build_mlp(
            ...
            self.teacher_latent_dim * 2,
            activation_name,
        )

        self.student_policy_head = self._build_mlp(
            self.teacher_latent_dim + 1,
    ...
    def act_inference(
        ...
        uncertainty = cov.sum(dim=-1, keepdim=True)
        policy_input = torch.cat([mean, uncertainty], dim=-1)
        actions_mean = self.student_policy_head(policy_input)
        if return_latent:
            return actions_mean, mean
        return actions_mean
